			...
			{playlists.items.map(item => {
                return (
                    <div className="playlist_entry">
						<img className="playlist_img" alt={item.name + " playlist image"} width="50px" height="50px" src={item.images.length ? item.images[item.images.length-1].url : 
						require("./imgNotFound.jpg")}/>
						<span className="playlist_text">{item.name} {item.description} {item.owner.display_name}</span>
						<br/>
					 </div>
                )
           })}
...
function ProgressBar(){
	...
	<div id="progressBar">
		...
		<br/>
		<progress id="progressBarMeter"value={progvalue} max={maxvalue}>{progvalue/maxvalue}%</progress>
		<br/>
		<span id="progressBarBack">Back  </span>
		<span id="progressBarPause" style={{display: paused ? "none" : "inline"}}>Pause  </span>
		<span id="progressBarPlay" style={{display: !paused ? "none" : "inline"}}>Play  </span>
			...
			{tracks.items.map(item => {
                return (
                    <div className="tracks_entry">
						<img className="tracks_img" alt={item.track.name + " tracks image"} src={item.track.images ? item.track.images[item.track.images.length-1].url : 
						require("./imgNotFound.jpg")}/>
						<span className="tracks_text">{item.track.name} {item.track.artists.name}</span>
						<br/>
					 </div>
                )
           })}
